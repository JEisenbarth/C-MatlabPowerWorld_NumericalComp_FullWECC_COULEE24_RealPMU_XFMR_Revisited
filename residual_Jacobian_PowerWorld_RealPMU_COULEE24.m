function [residual,J] = residual_Jacobian_PowerWorld_RealPMU_COULEE24(theta,theta_indicies,index,data_event,filenamedyd,gentpj,esst5b,pss2b,PQ_Flag,SimAuto,percentnominal,filename_RunAux,filename_PlayInCase,filename_SetupAux,Ipmu,Vpmu,XFMR,hygovr,oel1,Line)
...
for m=1:length(theta)
    if theta_indicies(m,1)==1
        theta_nominal(m)=gentpj(index.gentpj(theta_indicies(m,2)));
    elseif theta_indicies(m,1)==2
        theta_nominal(m)=esst5b(index.esst5b(theta_indicies(m,2)));
    elseif theta_indicies(m,1)==3
        theta_nominal(m)=oel1(index.oel1(theta_indicies(m,2)));
    elseif theta_indicies(m,1)==4
        theta_nominal(m)=pss2b(index.pss2b(theta_indicies(m,2)));
    elseif theta_indicies(m,1)==5
        theta_nominal(m)=hygovr(index.hygovr(theta_indicies(m,2)));
    elseif theta_indicies(m,1)==6
        theta_nominal(m)=XFMR(index.XFMR(theta_indicies(m,2)));
    elseif theta_indicies(m,1)==7
        theta_nominal(m)=Line(index.Line(theta_indicies(m,2)));
    end
end
...
for m=1:length(theta)
    if theta_indicies(m,1)==1
        gentpj(index.gentpj(theta_indicies(m,2)))=theta(m);
    elseif theta_indicies(m,1)==2
        esst5b(index.esst5b(theta_indicies(m,2)))=theta(m);
    elseif theta_indicies(m,1)==3
        oel1(index.oel1(theta_indicies(m,2)))=theta(m);
    elseif theta_indicies(m,1)==4
        pss2b(index.pss2b(theta_indicies(m,2)))=theta(m);
    elseif theta_indicies(m,1)==5
        hygovr(index.hygovr(theta_indicies(m,2)))=theta(m);
    elseif theta_indicies(m,1)==6
        XFMR(index.XFMR(theta_indicies(m,2)))=theta(m);
    elseif theta_indicies(m,1)==7
        Line(index.Line(theta_indicies(m,2)))=theta(m);
    end
end
theta
%% Write dyd and run simulation
[data]=PowerWorld_WriteDYD_Run_COULEE24_RealPMU(filenamedyd,gentpj,esst5b,pss2b,SimAuto,filename_RunAux,filename_PlayInCase,filename_SetupAux,Ipmu,Vpmu,XFMR,hygovr,oel1,Line);
...
ndxP=PWFind(data,'Branch ',' 40287 41744 1 ','MW From');
ndxQ=PWFind(data,'Branch ',' 40287 41744 1 ','Mvar From');
...
for x=1:length(theta)
    ...
    if theta_indicies(x,1)==1
        gentpj(index.gentpj(theta_indicies(x,2)))=theta(x)+delta(x);
    elseif theta_indicies(x,1)==2
        esst5b(index.esst5b(theta_indicies(x,2)))=theta(x)+delta(x);
    elseif theta_indicies(x,1)==3
        oel1(index.oel1(theta_indicies(x,2)))=theta(x)+delta(x);
    elseif theta_indicies(x,1)==4
        pss2b(index.pss2b(theta_indicies(x,2)))=theta(x)+delta(x);
    elseif theta_indicies(x,1)==5
        hygovr(index.hygovr(theta_indicies(x,2)))=theta(x)+delta(x);
    elseif theta_indicies(x,1)==6
        XFMR(index.XFMR(theta_indicies(x,2)))=theta(x)+delta(x);
    elseif theta_indicies(x,1)==7
        Line(index.Line(theta_indicies(x,2)))=theta(x)+delta(x);
    end
    
    %% Write dyd and run simulation for residual for Jacobian calc.
    [data_Jacobian]=PowerWorld_WriteDYD_Run_COULEE24_RealPMU(filenamedyd,gentpj,esst5b,pss2b,SimAuto,filename_RunAux,filename_PlayInCase,filename_SetupAux,Ipmu,Vpmu,XFMR,hygovr,oel1,Line);
    ...
    %% Find indicies for residual calculations then calc residual.
    ndxP=PWFind(data_Jacobian,'Branch ',' 40287 41744 1 ','MW From');
    ndxQ=PWFind(data_Jacobian,'Branch ',' 40287 41744 1 ','Mvar From');
...
%     
    ...
    if theta_indicies(x,1)==1
        gentpj(index.gentpj(theta_indicies(x,2)))=theta(x);
    elseif theta_indicies(x,1)==2
        esst5b(index.esst5b(theta_indicies(x,2)))=theta(x);
    elseif theta_indicies(x,1)==3
        oel1(index.oel1(theta_indicies(x,2)))=theta(x);
    elseif theta_indicies(x,1)==4
        pss2b(index.pss2b(theta_indicies(x,2)))=theta(x);
    elseif theta_indicies(x,1)==5
        hygovr(index.hygovr(theta_indicies(x,2)))=theta(x);
    elseif theta_indicies(x,1)==6
        XFMR(index.XFMR(theta_indicies(x,2)))=theta(x);
    elseif theta_indicies(x,1)==7
        Line(index.Line(theta_indicies(x,2)))=theta(x);
    end
...
% hold off
% 
% title([num2str(data_event.datatimes(1)),' to ',num2str(data_event.datatimes(2)),' P Plot'])
% % title('P Plot')
% legend();
% grid
% 
